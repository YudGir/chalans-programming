...
for angka in angka_user:
    # total_banyak_angkauser = angka_user.count(angka)
    print(angka_user.count(angka))

    if (angka_user.count(angka) > angka_user.count(angka + 1)):
        angka_user_terbanyak_muncul = angka
...
print(f"Angka yang paling banyak muncul (modus): {angka_user_terbanyak_muncul}")
